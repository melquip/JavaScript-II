...
let fullNames = [];
//fullNames = runners.map((runner) => runner.first_name + ' ' + runner.last_name);
...
// Problem 1
// We need to show everyone who donated and how much. Create an array to automate the string generation.
let donationLines = runners.map((runner) => `The company ${runner.company_name} has donated around ${(runner.donation*1).toFixed(2)}€!`);
console.log('Problem 1:', donationLines);
// Problem 2
// We need to find out who donated the most money. Sort the runners by the ammount donated.
let orderByHighestDonation = runners.concat().sort((runner, nextRunner) => runner.donation > nextRunner.donation ? -1 : 1);
console.log('Problem 2:', orderByHighestDonation);
// Problem 3
// We'll be raising the donations by a random % of money between 10% and 30% on each donation. Create a new array and add a new property to each runner with the final value and percentage added.
let trustFund = runners.map((runner) => {
	let buffer = ((Math.random()*30) + 10).toFixed(0);
	runner.percentage_buffer = buffer + '%';
	runner.final_buffer = parseFloat((runner.donation * (buffer/100)).toFixed(2));
	return runner;
});
console.log('Problem 3:', trustFund);